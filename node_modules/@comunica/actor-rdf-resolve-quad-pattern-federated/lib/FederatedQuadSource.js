...
class FederatedQuadSource {
    ...
    static skolemizeTerm(term, sourceId) {
        if (term.termType === 'BlankNode') {
            return new data_factory_1.BlankNodeScoped(`bc_${sourceId}_${term.value}`, DataFactory.namedNode(`${FederatedQuadSource.SKOLEM_PREFIX}${sourceId}:${term.value}`));
        }
        return term;
    }
    /**
    ...
    matchLazy(subject, predicate, object, graph) {
        ...
        const it = new asynciterator_1.UnionIterator(proxyIt.clone(), { autoStart: false });
